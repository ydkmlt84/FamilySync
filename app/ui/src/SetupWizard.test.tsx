...
import { InitialSyncStep, SetupWizard } from "./SetupWizard";
import { api, type SyncJobStatus } from "./api";

describe("SetupWizard authentication recovery", () => {
  it("asks for sign-in when an admin exists but this browser has no session", () => {
    render(
      <SetupWizard
        busy={false}
    ...
    expect(
      screen.getByText(
        "Sign back in with the linked Plex administrator account to continue setup.",
      ),
    ).toBeInTheDocument();

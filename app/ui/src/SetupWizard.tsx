...
export function SetupWizard({
  ...
  const [step, setStep] = useState<WizardStep>(
    needsFirstAdmin ? "welcome" : currentUser ? "server" : "admin",
  );
  ...
  }, [currentUser, needsFirstAdmin, step]);
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-[#cbd5e1]">
        {needsFirstAdmin ? (
          <>
            Sign in with the Plex account that <strong>owns the server</strong>{" "}
            you want FamilySync to manage. The first owner to link becomes the
            admin.
          </>
        ) : (
          <>
            Sign back in with the linked Plex administrator account to continue
            setup.
          </>
        )}
      </p>
      ...
      >
        ...
        {busy ? "Connecting" : needsFirstAdmin ? "Link with Plex" : "Sign in"}
        <ArrowRight size={17} />
      </button>
      {pinCode && (

...
export class AuthController {
  ...
  async pollPin(
    ...
    const existingUser = await this.users.existsByPlexUserId(
      resolved.plexUserId,
    );
    ...
    const action = existingUser ? "signed-in" : "linked";
    this.logger.log(
      existingUser
        ? `Signed in Plex user ${user.plexUsername} (${user.plexUserId}).`
        : `Linked new Plex user ${user.plexUsername} (${user.plexUserId}).`,
    );
    return { linked: true, action, user: toLinkedUserResponse(user) };
  }

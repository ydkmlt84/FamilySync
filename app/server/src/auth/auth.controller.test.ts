...
function makeController({
  ...
  const users = {
    existsByPlexUserId: vi.fn().mockResolvedValue(existingUser),
...
describe("AuthController setup protection", () => {
  ...
  it("links additional non-owner users once an admin exists", async () => {
    ...
    expect(result).toMatchObject({ linked: true, action: "linked" });
    expect(users.upsertFromPlex).toHaveBeenCalled();
  });

  it("signs in an existing linked Plex user", async () => {
    const { controller, users } = makeController({
      existingUser: true,
      resolved: {
        plexUserId: "1",
        plexUsername: "Owner",
        serverAccessToken: "updated-server-token",
        accountToken: "updated-account-token",
        serverClientIdentifier: "server",
        serverName: "Home",
        isAdmin: true,
      },
    ...
    await expect(
      ...
      linked: true,
      action: "signed-in",
      user: { plexUsername: "Owner" },
    });
    expect(users.existsByPlexUserId).toHaveBeenCalledWith("1");
    expect(users.upsertFromPlex).toHaveBeenCalled();
  });

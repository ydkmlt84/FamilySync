...
import {
  ...
  METADATA_SYNC_LAST_RUN_SETTING,
  METADATA_SYNC_JOB_SETTING,
  RATING_SYNC_LAST_RUN_SETTING,
  RATING_SYNC_JOB_SETTING,
  SettingsService,
  TAG_SYNC_LAST_RUN_SETTING,
...
export class SyncService {
  ...
  startFullSync(trigger: SyncJobStatus["trigger"] = "manual"): SyncJobStatus {
    return this.startJob("Full library sync", "full", trigger, (job) =>
      this.runFullSync(job),
    );
  }

  startUserSync(userId: string): SyncJobStatus {
    return this.startJob("User rating sync", "user", "manual", (job) =>
      this.runUserSync(job, userId),
    );
  }

  startTagSync(trigger: SyncJobStatus["trigger"] = "manual"): SyncJobStatus {
    return this.startJob("Tag sync", "tags", trigger, (job) =>
      this.runTagSync(job),
    );
  }

  startMetadataSync(
    trigger: SyncJobStatus["trigger"] = "manual",
  ): SyncJobStatus {
    ...
    return (
      running ??
      this.startJob("Plex metadata refresh", "metadata", trigger, (job) =>
  ...
  private async recordLastRun(job: SyncJobStatus): Promise<void> {
    const settingKey =
      job.scope === "full"
        ? RATING_SYNC_LAST_RUN_SETTING
        : job.scope === "tags"
    ...
    try {
      await this.settings.setJson(settingKey, {
        startedAt: job.startedAt.toISOString(),
        trigger: job.trigger,
      } satisfies JobLastRun);
    } catch (error) {
      this.logger.warn(
        `Could not persist last run for ${job.scope}: ${
  ...
  async getJobSettings(): Promise<{
    ...
    const [
      ...
      this.settings.getJson<CronJobSettings>(
        RATING_SYNC_JOB_SETTING,
      ...
      this.settings.getJson<JobLastRun | undefined>(
        RATING_SYNC_LAST_RUN_SETTING,
        undefined,
      ),
      this.settings.getJson<JobLastRun | undefined>(
    ...
    return {
      ratingSync: {
        ...ratingSync,
        ...(ratingLastRun ? { lastRun: ratingLastRun } : {}),
      },
      tagSync: {
        ...tagSync,
        ...(tagLastRun ? { lastRun: tagLastRun } : {}),
      },
      metadataSync: {
        ...metadataSync,
        ...(metadataLastRun ? { lastRun: metadataLastRun } : {}),
      },
    };
  }

...
describe("RatingsService aggregation and display", () => {
  ...
  it("returns names only for linked users who rated a carousel item", async () => {
    const { service, userRatings, users } = makeRatingsService();
    ...
    userRatings.find.mockResolvedValue([
      { plexUserId: "1", rating: 10 },
      { plexUserId: "2", rating: null },
      { plexUserId: "4", rating: 10 },
    ]);

    await expect(service.listCarouselRaterNames("perfect")).resolves.toEqual([
      "Alex",
    ]);
    expect(userRatings.find).toHaveBeenCalledWith({
      where: { ratingKey: "perfect" },
    });
  });

  it("searches cached titles and deduplicates user rows", async () => {
    const { service, userRatings } = makeRatingsService();
    userRatings.find.mockResolvedValue([
      {
        ratingKey: "1",
    ...
    await expect(service.searchCachedMedia(" movie ")).resolves.toEqual([
      {
        ratingKey: "1",
        title: "The Movie",
        mediaType: "movie",
    ...
    await expect(service.searchCachedMedia("m")).resolves.toEqual([]);
  });

...
describe("RatingsController", () => {
  it("requires authentication for sensitive media endpoints", () => {
    ...
    expect(
      Reflect.getMetadata(
        GUARDS_METADATA,
        RatingsController.prototype.carouselRaters,
      ),
    ).toEqual([AuthGuard, AdminGuard]);
    expect(
      Reflect.getMetadata(GUARDS_METADATA, RatingsController.prototype.search),
    ).toEqual([AuthGuard, AdminGuard]);
  });
  ...
  it("delegates rating and override operations", async () => {
    const ratings = {
      ...
      isPublicCarouselItem: vi.fn().mockResolvedValue(true),
      listCarouselRaterNames: vi.fn().mockResolvedValue(["Alex"]),
      searchCachedMedia: vi.fn().mockResolvedValue(["result"]),
    ...
    await expect(controller.carouselRaters("1")).resolves.toEqual({
      names: ["Alex"],
    });
    await expect(controller.search("movie")).resolves.toEqual(["result"]);
    expect(ratings.searchCachedMedia).toHaveBeenCalledWith("movie");
